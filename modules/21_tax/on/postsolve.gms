...
p21_taxrevBio0(ttot,regi) = (v21_tau_bio.l(ttot) * vm_pebiolc_price.l(ttot,regi) + p30_bioen_price_coemi(ttot,regi)) * vm_fuExtr.l(ttot,regi,"pebiolc","1");
p21_implicitDiscRate0(ttot,regi) = sum(ppfKap(in),  p21_implicitDiscRateMarg(ttot,regi,in)  * vm_cesIO.l(ttot,regi,in) );
***DK: for reporting only; LM: FIXME, tax according to co-emission factor missing
