...
var_price_shapes = c("Internal|Price|Biomass|MAgPIE (US$2005/GJ)" = 0,
...
for (r in regions){
  ...
  if(r!= "GLO") {
    ...
    swlatex(pdf,plot.title)
    # print(getNames(csv)[grepl("Price|Biomass", getNames(csv), fixed=T)])
    var_price = c("Internal|Price|Biomass|MAgPIE (US$2005/GJ)","Internal|Price|Biomass|Emulator presolve (US$2005/GJ)","Internal|Price|Biomass|Emulator presolve shifted (US$2005/GJ)","Internal|Price|Biomass|Emulator shifted (US$2005/GJ)")
