...
*** ---------------------------------------------------------------------------------------------------------------
*** new adpation of peakBudg formulation that works with several CO2 price path realizations of module 45 ---------
*** it results in a peak budget with linear increase by 2$/yr afterwards
*** in contrast to realization cm_iterative_target_adj = 9, here the peak budget contains all GHGs, not only CO2.
*** ATTENTION: in order to keep it simple, variable names are not adjusted to realization 9, e.g. p_actualbudgetco2
*** is used here as well. However, it does now contain the total amount of GHGs and not only CO2, as the name might
*** suggest.
*** ---------------------------------------------------------------------------------------------------------------

if(cm_iterative_target_adj eq 10,
*RP* Update tax levels/ multigasbudget values to reach the peak budget, with a linear increase afterwards given by cm_taxCO2inc_after_peakBudgYr
*** The PeakBudgYr is found automatically by the algorithm (within the time window 2040-2100)
...
*** -----B2: if there was a flip-floping of cm_peakBudgYr in the previous iterations, try to overome this by adjusting the CO2 price path after the peaking year	
    if (o_delay_increase_peakBudgYear(iteration) = 1,   
      ...
      loop(ttot$(ttot.val eq cm_peakBudgYr),  !! set ttot to the current peakBudgYr 
        loop(t2$(t2.val eq pm_ttot_val(ttot+1)),  !! set t2 to the following time step
          o_factorRescale_taxCO2_afterPeakBudgYr(iteration) = 1 + max(sum(regi2,vm_co2eq.l(ttot,regi2))/sum(regi2,vm_co2eq.l("2015",regi2)),-0.75) ; 
		  !! this was inspired by Christoph's approach. This value is 1 if emissions in the peakBudgYr are 0; goes down to 0.25 if emissions are <0 and approaching the size of 2015 emissions, and > 1 if emissions > 0. 
